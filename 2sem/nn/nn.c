...
int main(int argc, char ** argv){
	init_file(argv[1], 10000000, 0.39);
	unsigned long long struc_net[3] = {40*40, 40*40*10, 10};
	struct neron_net * nn = neron_net_init(3, struc_net, argv[1]);

	double result[610][10] = {0};
	double ** data_mas = (double **)malloc(sizeof(double *) * 610);
	for (int i = 0; i < 610; ++i){
		data_mas[i] = getdata(strcat(itoa(i),".txt"));
		result[i][i % 10] = 1;
	}

	// double * data1 = getdata(argv[2]);
	...
	// double result4[9] = {0.,0.,0.,1.,0.,0.,0.,0.,0.};
	// double result5[9] = {0.,0.,0.,0.,1.,0.,0.,0.,0.};
	// double result6[9] = {0.,0.,0.,0.,0.,1.,0.,0.,0.};
	// double result7[9] = {0.,0.,0.,0.,0.,0.,1.,0.,0.};
	// double result8[9] = {0.,0.,0.,0.,0.,0.,0.,1.,0.};
	// double result9[9] = {0.,0.,0.,0.,0.,0.,0.,0.,1.};
	
	for (long long i = 0; 1; ++i){
		for (int j = 0; j < 610; ++j){
			neron_learn(nn, data_mas[i], result[i%10]);	
		}
		if(i % 300 == 0){
			save_file(argv[1], nn);
			system("cls");
			...
			for (int j = 0; j < 10; ++j){
				printf("\nansvers %d:\n",j);
				neron_start(nn, data[j]);
				for (int i = nn->size_ansvers - 10; i < nn->size_ansvers; ++i){
